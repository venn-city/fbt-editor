...
import CurrentUser from "@entities/authentication/CurrentUser";
...
export default class AuthenticationService {
  ...
  constructor() {
    ...
    this.client = new OAuth2Client(
      ...
      this.authenticationSettings.secret
    );
  }
    ...
    if (payload) {
      if (!this.usersProvider.contains(payload.email!)) {
        return new AuthenticationResult(
          null,
          UNAUTHORIZED,
          "You don't have rights to access! Please contact to project owner"
        );
      }
      if (payload?.aud !== this.authenticationSettings.clientId) {
        return new AuthenticationResult(
          null,
          UNAUTHORIZED,
          "Wrong recipient, payload audience"
        );
      }
