...
export default class TranslationFilesProvider {
  ...
  public async getTranslationSourceFile(
    projectId: string,
    fileId: string
  ): Promise<TranslationSourceFile> {
    const sourceFileName = this.projectProvider.getSourceFileName(projectId);
    const translationInputId = this.getTranslationInputId(
      fileId,
      sourceFileName
    );
    let fileContent: AWS.S3.Types.GetObjectOutput;
    try {
      fileContent = await this.s3BucketRepository.getBucketObjectContent(
        projectId,
        translationInputId
      );
    } catch (e) {
      throw new Error(
        `File ${path.basename(sourceFileName)} does not exist in folder.`
      );
    }
    let translationFile: TranslationSourceFile;
    try {
      translationFile = JSON.parse(
  ...
  public async getTranslationTargetFile(
    projectId: string,
    ...
    throw new Error(
      `File ${path.basename(projectFileId)} does not have valid format.`
    );
  }
}
