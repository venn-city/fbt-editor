...
class ProjectItemContentController {
  ...
  public async getfileContent(req: Request, res: Response) {
    ...
    if (projectId && fileId) {
      const file = await this.projectFileContentProvider.getProjectFileContent(
        projectId,
        fileId
      );
  ...
  public async updatefileContent(req: Request, res: Response) {
    ...
    if (projectFileContent) {
      const translationFile = await this.translationFileContentProvider.getFileContent(
        projectFileContent
      );
      await this.projectItemProvider.createFile(
