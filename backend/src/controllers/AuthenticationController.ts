import { AuthenticationClientData } from '@entities/AuthenticationClientData';
...
class AuthenticationController {
    ...
    public async getClientData(req: Request, res: Response)  {
        return this.responseProvider.createSuccessfullResponse(res, new AuthenticationClientData(this.authenticationSettings.clientId));
    }

    public async login(req: Request, res: Response)  {
        const loginUserRequest = req.body as LoginUserRequest;
        if (loginUserRequest) {
            const authenticationResult: AuthenticationResult = await this.authenticationService.login(loginUserRequest.tokenId);
            if (authenticationResult.currentUser) {
                this.authenticationProvider.setAuthCookies(res, authenticationResult.currentUser.tokenId);
                return this.responseProvider.createSuccessfullResponse(res, authenticationResult.currentUser);
            }
            return this.responseProvider.createRequestResponse(res, authenticationResult.statusCode, authenticationResult.message);
        }
