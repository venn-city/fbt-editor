...
import ProjectContent from "@entities/project/ProjectContent";
import ProjectFile from "@entities/project/ProjectFile";
import ProjectFolder from "@entities/project/ProjectFolder";
...
class ProjectItemController {
  ...
  public async getAll(req: Request, res: Response) {
    ...
    if (projectId) {
      const data: AWS.S3.Types.ListObjectsV2Output = await this.bucketRepository.getBucketObjects(
        projectId,
        folderId
      );
      ...
      if (data.Contents) {
        projectFiles = data.Contents.filter(
          (file) => path.extname(file.Key!) === ".json"
        ).map((file) => {
      ...
      if (data.CommonPrefixes) {
        projectFolders = data.CommonPrefixes?.map(
          (folder) =>
            new ProjectFolder(folder.Prefix!, path.basename(folder.Prefix!))
        );
      }
      const projectContent: ProjectContent = new ProjectContent(
        projectId,
        projectFolders
          .concat(projectFiles)
    ...
    return this.responseProvider.createBadRequestResponse(
      res,
      "Invalid projectId or file ID."
    );
  }
  ...
  public async createItem(req: Request, res: Response) {
    ...
    return this.responseProvider.createBadRequestResponse(
      res,
      "Create item request has wrong body."
    );
  }
  ...
  public async uploadItem(req: Request, res: Response) {
    ...
    if (createItemRequest && file) {
      await this.projectItemProvider.uploadItem(
    ...
    return this.responseProvider.createBadRequestResponse(
      res,
      "Upload item request has wrong body."
    );
  }
  ...
  public async deleteItem(req: Request, res: Response) {
    ...
    if (deleteItemRequest) {
      await this.bucketRepository.deleteObject(
        deleteItemRequest.projectId,
    ...
    return this.responseProvider.createBadRequestResponse(
      res,
      "Delete item request has wrong body."
    );
  }
}
