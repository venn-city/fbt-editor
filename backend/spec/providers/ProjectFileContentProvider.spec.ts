...
describe("ProjectFileContentProvider", () => {
  ...
  it(`should return s3 with settings`, async (done) => {
    ...
    spyOn(
      TranslationFilesProvider.prototype,
      "getTranslationTargetFile"
    ).and.returnValue(Promise.resolve(translationTargetFile));

    const result: ProjectFileContent = await projectFileContentProvider.getProjectFileContent(
      projectId,
      projectFileId
    );
