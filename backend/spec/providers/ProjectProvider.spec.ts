...
describe('ProjectProvider', () => {
    ...
    const projectConfiguration: ProjectConfiguration = new ProjectConfiguration(projectId, bucketName, "accessKeyId", "secretAccessKey", "", sourceFileName);
    const projectsConfiguration: ProjectConfiguration[] = [projectConfiguration];
    let projectProvider: ProjectProvider;

    beforeAll((done) => {
        spyOn(projectsCache, 'getProjectsConfiguration').and.returnValue(projectsConfiguration);
    ...
    describe('getProject', () => {
        it(`should return project configuration when projectId exists`, (done) => {
            const result: ProjectConfiguration|null = projectProvider.getProject(projectId);
        ...
        it(`should return null when projectId does not exist`, (done) => {
            expect(()=> projectProvider.getProject("test")).toThrowError("Project (test) does not exist!");
            done();
        });
    });
