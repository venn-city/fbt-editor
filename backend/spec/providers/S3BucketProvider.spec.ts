...
describe('S3BucketProvider', () => {
    ...
    it(`should return s3 with settings`, (done) => {
        const projectConfiguration = new ProjectConfiguration("testName", "test", "accessKeyId", "secretAccessKey", "", "");
        spyOn(ProjectProvider.prototype, 'getProject').and.returnValue(projectConfiguration);

        const s3bucket = bucketProvider.createS3Bucket(projectId);

        expect(s3bucket.config.credentials?.secretAccessKey).toEqual(secretAccessKey);
