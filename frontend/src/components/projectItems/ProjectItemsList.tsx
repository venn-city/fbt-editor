...
import { CreateNewFolder, KeyboardBackspace, NoteAdd, SaveAlt, Sort } from '@material-ui/icons';
...
import {
  ...
  getIsLoading, getProjectItemsList,

  reverseProjectItemsList, uploadSourceFile,
} from '../../store/duck/projectItems';
import { CreateItemRequest, ProjectItem, ProjectItemType } from '../../store/entities';
import { getProjectItemParentFolderPath, getProjectPath } from '../../utils/pathNavigation';
...
const ProjectItemsList = ({ header }: ProjectItemsListProps) => {
  ...
  const [openCreateProjectItemDialog, setOpenCreateProjectItemDialog] = useState(false);
  ...
  const handleProjectItemSave = (name: string, targetLanguage: string | undefined) => {
    dispatch(createProjectItem({
      projectId,
  ...
  return (
    ...
    >
      ...
      <Grid container item className={classes.buttonContainer}>
        <IconButton aria-label="back" disabled={!folderId} onClick={onBackButtonClick}>
      ...
      <Grid container item alignItems="center" className={classes.projectList}>
        ...
        {projectItems.map((projectItem: ProjectItem) => <ProjectListItem key={projectItem.id} projectItem={projectItem} />)}
      </Grid>
