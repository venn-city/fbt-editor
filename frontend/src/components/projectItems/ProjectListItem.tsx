...
import { getFilePath, getFolderPath, getProjectPath } from '../../utils/pathNavigation';
...
const ProjectListItem = ({ projectItem }: ProjectItemProps) => {
  ...
  const deleteAction = () => dispatch(deleteProjectItem({
    projectId,
  ...
  const onFolderClick = (folderId: string) => onEntityClick(getFolderPath(folderId), push);
  const onFileClick = (fileId: string) => onEntityClick(getFilePath(fileId), push, '/file');
    ...
    <Grid container item className={classes.root} justify="space-between">
      ...
      <Grid item>
        <ControlButtonGroup
          buttons={
            [
              isFile && !projectItem.readonly && { name: 'edit', onClick: () => onFileClick(projectItem.id), tooltip: 'Edit file' },
              {
                name: 'delete',
