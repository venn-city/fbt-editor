...
import {
  ...
  getError,
  getIsCreatingProjectItem, getIsLoading,
  getProjectItems,
  getProjectItemsList,
  ProjectItemsAction,
  projectItemsReducer,
...
import { CreateItemRequest, DeleteItemRequest, ProjectItem } from '../store/entities';
...
describe('projectItems actions', () => {
  it('fetchProjectItemsList', () => {
    const { type, projectId, projectItemId } = fetchProjectItemsList(mockState.projectId, mockState.projectItemId);
  ...
  it('fetchProjectItemsListSuccess', () => {
    const { projectItems, type } = fetchProjectItemsListSuccess(mockState.projectItems);
  ...
  it('createProjectItem', () => {
    ...
    expect(action.сreateItemRequest!.targetLanguage).toBe(mockState.targetLanguage);
    expect(action.сreateItemRequest!.parentFolderId).toBe(mockState.parentFolderId);
  });
...
describe('projectItems reducer', () => {
  ...
  describe('when receiving not relative action', () => {
    it('should return default state', () => {
      const newState = projectItemsReducer(projectItemsState, { type: 'RANDOM_NAME' } as ProjectItemsAction);
  ...
  describe('start fetching projectItems list', () => {
    describe('on success', () => {
      const newState = projectItemsReducer(
        projectItemsState,
        {
...
describe('selectors', () => {
  ...
  describe('getProjectItemsList', () => {
    it('should return projectItems list', () => {
      expect(getProjectItemsList(state)).toEqual(Object.values(projectItemsState.projectItemsList));
    });
  });

  describe('getIsCreatingProjectItem', () => {
    it('should return status of creating projectItem', () => {
      expect(getIsCreatingProjectItem(state)).toEqual(projectItemsState.isUpdatingProjectItems);
    });
  });
