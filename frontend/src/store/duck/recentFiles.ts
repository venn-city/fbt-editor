...
export const fetchRecentFilesList = ():RecentFilesAction => ({ type: FETCH_RECENT_FILES_LIST } as RecentFilesAction);
export const fetchRecentFilesListSuccess = (recentFiles: RecentFile[]):RecentFilesAction =>
  ({ type: FETCH_RECENT_FILES_LIST_SUCCESS, recentFiles } as RecentFilesAction);
export const fetchRecentFilesListFailure = (error: ApiError):RecentFilesAction =>
  ({ type: FETCH_RECENT_FILES_LIST_FAILURE, error } as RecentFilesAction);
export const reverseRecentFilesList = ():RecentFilesAction => ({ type: REVERSE_RECENT_FILES_LIST } as RecentFilesAction);

export const recentFilesReducer = (state: RecentFilesState = INITIAL_STATE, action: RecentFilesAction ) => {
  switch (action.type) {
...
export const getRecentFiles = (state: {recentFiles: RecentFilesState}): RecentFilesState => state.recentFiles;
export const getRecentFilesList = (state: {recentFiles: RecentFilesState}) => getRecentFiles(state).recentFilesList;
export const getIsLoading = (state: {recentFiles: RecentFilesState}) => getRecentFiles(state).isLoading;
export const getError = (state: {recentFiles: RecentFilesState}) => getRecentFiles(state).error;

export function* fetchRecentFiles():Generator {
  try {
    // @ts-ignore
    const recentFiles:RecentFile[] = yield* requestAPI(API.fetchRecentFilesList);
