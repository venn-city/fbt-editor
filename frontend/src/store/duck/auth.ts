...
export const FETCH_AUTH_CLIENT_DATA_SUCCESS =
  "auth/FETCH_AUTH_CLIENT_DATA_SUCCESS";
export const FETCH_AUTH_CLIENT_DATA_FAILURE =
  "auth/FETCH_AUTH_CLIENT_DATA_FAILURE";
...
export const loginUserFailure = (error: ApiError): UsersStateAction =>
  ({ type: LOGIN_USER_FAILURE, error } as UsersStateAction);
export const logout = (): UsersStateAction =>
  ({ type: LOGOUT } as UsersStateAction);
...
export const fetchAuthClientDataFailure = (error: ApiError): UsersStateAction =>
  ({ type: FETCH_AUTH_CLIENT_DATA_FAILURE, error } as UsersStateAction);
...
export const getUsersState = (state: { users: UsersState }): UsersState =>
  state.users;
export const getCurrentUser = (state: {
  users: UsersState;
}): CurrentUser | null => getUsersState(state).currentUser;
export const getCurrentUserEmail = (state: {
  users: UsersState;
}): string | undefined => getUsersState(state).currentUser?.email;
export const getIsLoggedIn = (state: { users: UsersState }): boolean =>
  getUsersState(state).isLoggedIn;
export const getError = (state: { users: UsersState }): ApiError | null =>
  getUsersState(state).error;
export const getAuthClientData = (state: {
  users: UsersState;
}): AuthClientData => getUsersState(state).authClientData;
...
export function* authSaga() {
  yield all([watchLogin(), watchFetchAuthClientData()]);
}
