...
export const fetchProjectsList = ():ProjectsAction => ({ type: FETCH_PROJECTS_LIST } as ProjectsAction);
export const fetchProjectsListSuccess = (projects: Project[]):ProjectsAction => ({ type: FETCH_PROJECTS_LIST_SUCCESS, projects } as ProjectsAction);
export const fetchProjectsListFailure = (error: ApiError):ProjectsAction => ({ type: FETCH_PROJECTS_LIST_FAILURE, error } as ProjectsAction);
export const reverseProjectsList = ():ProjectsAction => ({ type: REVERSE_PROJECTS_LIST } as ProjectsAction);

export const projectsReducer = (state: ProjectsState = INITIAL_STATE, action: ProjectsAction) => {
  switch (action.type) {
...
export const getProjects = (state: { projects: ProjectsState }):ProjectsState => state.projects;
export const getProjectsList = (state: { projects: ProjectsState }):Project[] => getProjects(state).projectsList;
export const getIsLoading = (state: { projects: ProjectsState }):boolean => getProjects(state).isLoading;
export const getError = (state: { projects: ProjectsState }):Error|null => getProjects(state).error;
