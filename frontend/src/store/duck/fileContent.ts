...
import { ProjectFileContent, ProjectFileItem, UpdateFileItemData } from "./../entities";
...
export const fetchFileContent = (
  projectId: string,
  fileId: string,
):FileContentAction => ({ type: FETCH_FILE_CONTENT, projectId, fileId } as FileContentAction);

export const fetchFileContentSuccess = (projectFileItems: ProjectFileItem[], targetLanguage: string):FileContentAction =>
  ({ type: FETCH_FILE_CONTENT_SUCCESS, projectFileItems, targetLanguage } as FileContentAction);
export const fetchFileContentFailure = (error: ApiError):FileContentAction => ({ type: FETCH_FILE_CONTENT_FAILURE, error } as FileContentAction);
...
):FileContentAction => ({ type: UPDATE_FILE_CONTENT, projectId, fileId, targetLanguage, projectFileItems } as FileContentAction);

export const updateFileContentSuccess = () => ({ type: UPDATE_FILE_CONTENT_SUCCESS });
export const updateFileContentFailure = (error: ApiError) => ({ type: UPDATE_FILE_CONTENT_FAILURE, error });

export const updateFileContentField = (updateFileItemData: UpdateFileItemData):FileContentAction =>
  ({ type: UPDATE_FILE_CONTENT_FIELD, updateFileItemData } as FileContentAction);
...
export const clearFilteredFileContent = ():FileContentAction => ({ type: CLEAR_FILTERED_FILE_CONTENT } as FileContentAction);
export const clearFileContent = ():FileContentAction => ({ type: CLEAR_FILE_CONTENT } as FileContentAction);
  ...
  switch (action.type) {
    ...
    case FETCH_FILE_CONTENT_SUCCESS:
      return {
        ...
        fileContentMap: action.projectFileItems!
          .reduce((acc: ProjectFileItem, fileContent: ProjectFileItem) => ({ ...acc, [fileContent.id]: fileContent }), {} as ProjectFileItem),
      };
      ...
      return {
        ...
        fileContentMap: {
          ...
          [action.updateFileItemData!.id]: {
            ...
            target: action.updateFileItemData!.value,
          },
        },
      };
    ...
    case SET_FILTERED_FILE_CONTENT:
      return {
        ...
        filteredFileContentList: Object.values(state.fileContentMap)
          .filter((projectFileItem:ProjectFileItem) => projectFileItem.source.toLowerCase().includes(action.searchTerm!)),
      };
...
export const getFileContent = (state: {fileContent: FileContentState}):FileContentState => state.fileContent;
export const getFileContentList = (state: {fileContent: FileContentState}):ProjectFileItem[] => Object.values(getFileContent(state).fileContentMap);
export const getFilteredContentList = (state: {fileContent: FileContentState}):ProjectFileItem[] => getFileContent(state).filteredFileContentList;
export const getTargetLanguage = (state: {fileContent: FileContentState}):string => getFileContent(state).targetLanguage;
export const getIsLoading = (state: {fileContent: FileContentState}):boolean => getFileContent(state).isLoading;
export const getError = (state: {fileContent: FileContentState}):ApiError|null => getFileContent(state).error;

export function* fetchFileContentSaga({ projectId, fileId = '' }: FileContentAction) {
  try {
    // @ts-ignore
...
export function* updateFileContentSaga({ projectId, fileId = '', targetLanguage, projectFileItems }: FileContentAction) {
  try {
    // @ts-ignore
    yield* requestAPI(API.updateFileContent, { projectId, fileId, targetLanguage, projectFileItems } as ProjectFileContent);
